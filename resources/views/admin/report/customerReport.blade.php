      ...
      <div class="row" style="padding: 10px 10px 0px 10px;background: #06904d;margin:5px 5px 5px 0px;">
       ...
       <form action="{{ route('admin.customers') }}" method="get">
        ...
        <div class="row" style="text-align: left;">
          ...
          @if(Request::is('admin/cfa/orders/index'))
            <input type="hidden" class="form-control" name="selectedBranch" id="selectedBranch" value="{{ Auth::user()->branch_id }}">
           @endif
            ...
            <div class="col-md-7 col-lg-7 col-sm-7 col-xs-7" style="color: black;">
                <div id="date-range">
                    <div class="form-group data-custon-pick data-custom-mg" id="data_5">
                        <div class="input-daterange input-group" id="datepicker">
                          <input type="text" class="form-control" name="name" placeholder="search name" value="{{ $name }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table id="orders" class="table table-bordered">
              ...
              <tbody>
                ...
                @endphp
                  ...
                  <td>
                    <button type="button" class="btn btn-success"  data-id="{{ $key->id }}" data-name="{{ $key->name }}"  data-email="{{ $key->email }}" data-pass="{{ $key->password }}" onclick="editDesg_fun(this)"><i class="fas fa-pen"></i></button>
                    <a href="{{route('admin.customer.allorder', $key->id)}}" class="btn btn-primary" >View Order History</a>
                    <a href="{{route('admin.reviewReport')}}?user_id={{ $key->id }}" class="btn btn-primary" >View Review History</a>
                    <a href="{{route('admin.customer.delete', $key->id)}}" class="btn btn-danger" >delete</a>
                  </td>
                  </tr>
                @php
                    }
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title"> Customer Inforamtion</h4>
        </div>
        <form method="post" action="{{ route('admin.updateCustomer') }}">
          @csrf
        <div class="modal-body">
          <input type="hidden" name="customerId" id="customerId" class="form-control"><br>
          <input type="text" name="custname" id="custname" class="form-control"placeholder="Enter Customer Name." required><br><br>
          <input type="text" name="email" id="email" class="form-control" placeholder="Email" required><br><br>
          <input type="text" name="pass" id="pass" class="form-control" placeholder="For Reset Password">
        </div>
...
function editDesg_fun(d){
      var customerId = d.getAttribute("data-id");
      var custname=d.getAttribute("data-name");
      var email=d.getAttribute("data-email");
      var pass=d.getAttribute("data-pass");
