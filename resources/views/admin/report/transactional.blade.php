...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1>All Tranasctional Report</h1>
      </div>
    </div>
  </div><!-- /.container-fluid -->
</section>
  ...
  <div class="container-fluid">
    <div>
      <div class="row" style="padding: 10px 10px 0px 10px;background: #06904d;margin:5px 5px 5px 0px;">
       ...
       <form action="{{ route('admin.Transactional') }}" method="get">
          ...
          <div class="card-body">
            <table id="orders" class="table table-bordered">
              ...
              <tbody>
                ...
                @endphp
                @php
                    $orderCount =\App\Models\Order::where('user_id', $key->id)->get();
                @endphp
                @if(count($orderCount)>0)
                  ...
                  @php
                    $orderCount =\App\Models\User::where('id', $key->id)->get();
                    $name='';
                    $email='';
                    foreach($orderCount as $key1=>$value){
                        $name=$value->name;
                        $email=$value->email;
                    }
                 @endphp
                  ...
                  <td>{{ $email }}</td>
                  @php
                    $orderCount1 =\App\Models\Order::where('user_id', $key->id)->get();
                @endphp
                  <td>{{ count($orderCount1) }}</td>
                  <?php
                    $total=0;
                    $totalQty=0;
                    foreach($orderCount1 as $key=>$value){
                ...
                <div class="card-header">
                    <h2 class="card-title"><b>Overall Total Transactions = <span id="finaltotalshow">{{ $finalTotal }}</span></b></h2>
                </div>
